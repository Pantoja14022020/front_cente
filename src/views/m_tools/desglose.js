...
export default {
  ...
  data () {
    return {
      ...
      headers: [
        { text: 'Emitida por', value: 'usuarioEnvia', sortable: true  },
        { text: 'NUC',value: 'nucg', sortable: true},
        { text: 'Enviada a', value: 'dirSubRecibe', sortable: true  },
        { text: 'Distrito', value: 'distritoRecibe', sortable: true  },
        { text: 'Fecha',value: 'fechaDesglose', sortable: false },
  ...
  methods: {
    ...
    listar()
    {
      ...
      let configuracion= {headers : header};
        me.$cat.get('api/Desgloses/Listar/'+ me.infoCarpeta.rHechoId,configuracion).then(function(response){
    ...
    solicitarModificacion(info)
    {
      ...
      let header = { "Authorization" : "Bearer " + this.$store.state.token }
      let configuracion = { headers : header }
      me.$justiciarestaurativa.post('api/'+info.servicio ,
      { 
    ...
    mostrarFormularioModificacion (item,info) 
    {
      console.log("ENVIANDO FORMULARIO DE MODIFICACION 3");
      console.log(item);
      this.infoMod.tablaAMod='CAT_DESGLOSES';
      this.$refs.formModificacion.mostrar({
                                                  'rHechoId':item.rHechoId,
                                                  'rAtencionId':"00000000-0000-0000-0000-000000000000",
                                                  'registroId':item.idDesgloses
                                                },
                                          info
                                          );
    },
