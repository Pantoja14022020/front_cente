<script src="./personadesaparecida.js">
  ...
  <v-layout align-start>
    ...
    <v-flex v-if="showpage">
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title class="font-weight-regular" >Lista de Cédulas de persona desaparecida en Carpeta</v-toolbar-title>
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td>{{ props.item.padecimiento }}</td>
          <td>{{ props.item.etnia }}</td>
          <td>{{ props.item.grupoDelictivo }}</td>
          <td>{{ props.item.fechaHoraUltAvistamiento }}</td>
          <td class="justify-center layout px-0">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon class="mr-2" color="error" v-on="on" @click="mostrarFormularioModificacion(props.item,{title:'Borrar Cedula de Persona desaparecida',icon:'delete_forever',color:'red',operation:'PersonaDesap/Eliminar',buttonText:'Borrar',extraComponents:{}})">
                  delete_forever
                </v-icon>
              </template>
              <span>Eliminar Medida de Protección</span>
            </v-tooltip>
          </td>
        </template>
