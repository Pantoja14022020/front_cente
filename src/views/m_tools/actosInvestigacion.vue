<script src="./actosInvestigacion.js">
  ...
  <v-layout align-start>
    ...
    <v-flex v-if="showpage">
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title class="font-weight-regular" >Lista de Actos de Investigación en Carpeta</v-toolbar-title>
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td>{{ props.item.nodeSolicitud }}</td>
          <td>{{ props.item.uUsuario }}</td>
          <td>{{ props.item.status }}</td>
          <td class="justify-center layout px-0">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon class="mr-2" color="error" v-on="on" @click="mostrarFormularioModificacion(props.item,{title:'Borrar Acto de Investigación',icon:'delete_forever',color:'red',operation:'RActosInvestigacion/Eliminar',buttonText:'Borrar',extraComponents:{}},'CAT_RACTOSINVESTIGACION')">
                  delete_forever
                </v-icon>
              </template>
              <span>Eliminar Acto de Investigación</span>
            </v-tooltip>
