  ...
  <v-layout align-start>
    ...
    <v-flex v-if="showpage">
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title class="font-weight-regular" >Lista de Sábanas de Vehículo en Carpeta</v-toolbar-title>
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td>
            <div v-if="props.item.estadoRobado">
                                <span>Si</span>
                            </div>
                            <div v-else>
                                <span>No</span>
          ...
          <td class="justify-center layout px-0">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon class="mr-2" color="error" v-on="on" @click="mostrarFormularioModificacion(props.item,{title:'Borrar Sábana de Vehículo',icon:'delete_forever',color:'red',operation:'Vehiculos/Eliminar',buttonText:'Borrar',extraComponents:{}},'CAT_VEHICULO')">
                  delete_forever
                </v-icon>
              </template>
              <span>Eliminar Sábana de Vehículo</span>
            </v-tooltip>
