  ...
  <v-layout align-start>
    ...
    <v-flex v-if="showpage">
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title class="font-weight-regular" >Lista de Oficios de C5i en Carpeta</v-toolbar-title>
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td>{{ props.item.razonDescripcion }}</td>
          <td>{{ props.item.insumos }}</td>
          <td>{{ props.item.fechaSys }}</td>
          <td class="justify-center layout px-0">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon class="mr-2" color="error" v-on="on" @click="mostrarFormularioModificacion(props.item,{title:'Borrar Oficio C5i',icon:'delete_forever',color:'red',operation:'C5i/Eliminar',buttonText:'Borrar',extraComponents:{}},'CAT_C5FORMATOS')">
                  delete_forever
                </v-icon>
              </template>
              <span>Eliminar Oficio C5i</span>
            </v-tooltip>
