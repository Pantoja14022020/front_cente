  ...
  <v-layout align-start>
    ...
    <v-flex v-if="showpage">
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title class="font-weight-regular" >Lista de Bitacoras en Carpeta</v-toolbar-title>
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td>{{ props.item.fechasis }}</td>
          <td class="justify-center layout px-0">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon class="mr-2" color="error" v-on="on" @click="mostrarFormularioModificacion(props.item,{title:'Borrar Bitácora',icon:'delete_forever',color:'red',operation:'RBitacoras/Eliminar',buttonText:'Borrar',extraComponents:{}},'CAT_BITACORA')">
                  delete_forever
                </v-icon>
              </template>
              <span>Eliminar Bitácora</span>
            </v-tooltip>
