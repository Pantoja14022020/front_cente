  ...
  <v-layout align-start>
    ...
    <v-flex v-if="showpage">
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title class="font-weight-regular" >Lista de Remisiones de la Carpeta</v-toolbar-title>
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td>{{ props.item.fechasys }}</td>
          <td v-if="props.item.posicion==1" class="justify-center layout px-0">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon class="mr-2" color="error" v-on="on" @click="mostrarFormularioModificacion(props.item,{title:'Borrar Remision',icon:'delete_forever',color:'red',operation:'RemisionesUI/Eliminar',buttonText:'Borrar',extraComponents:{}},'CAT_REMISIONUI,CAT_RHECHO')">
                  delete_forever
                </v-icon>
              </template>
              <span>Eliminar Remisión</span>
            </v-tooltip>
