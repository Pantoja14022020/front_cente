  ...
  <v-layout align-start>
    ...
    <v-flex v-if="showpage">
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title class="font-weight-regular" >Lista de Medidas de Protección en Carpeta</v-toolbar-title>
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td>{{ props.item.medidaProtecion }}</td>
          <td>{{ props.item.duracion }}</td>
          <td class="justify-center layout px-0">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon class="mr-2" color="error" v-on="on" @click="mostrarFormularioModificacion(props.item,{title:'Borrar Medida de protección',icon:'delete_forever',color:'red',operation:'MedidasProteccion/Eliminar',buttonText:'Borrar',extraComponents:{}})">
                  delete_forever
                </v-icon>
              </template>
