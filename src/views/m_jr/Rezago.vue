      ...
      <v-list dense dark class="pt-0 primary" >
        ...
        <v-flex v-if="showpage">
            <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Expedientes con rezago.</v-toolbar-title>
                    ...
                    <template slot="items" class="white" slot-scope="props">
                        ...
                        <td>  
                            <div>
                                ...
                                {{inicioCarpeta(props.item.fechaElevaNuc)}}
                            </div>
                        </td> 
                        <td>  
                            <div>
                                <v-icon  class="mr-2"  >date_range</v-icon>
                                {{inicioDerivacion(props.item.fechaDerivacion)}}
                            </div>
                        </td>
                        <td>  
                            <div>
                                {{haySesionSatisfactoria(props.item.statusSesion,props.item.fechahorasesion)}}
                            </div>
                        </td>
                        <td>  
                            <div>
                                {{haySesionAcuerdoReparatorio(props.item.idAcuerdoReparatorio,props.item.fechaCelebracionAcuerdo)}}
                            </div>
                        </td>
                        <td>  
                            <div>
                                {{hayCumplimientoAcuerdo(props.item.statusPago,props.item.fechaseguimiento)}}
                            </div>
                        </td>                    
    ...
    export default {
        ...
        data(){
            return {      
                ...
                headers: [ 
                    { text: 'NUC', value: 'nucg',  },     
                    { text: 'Expediente', value: 'noExpediente'}, 
                    { text: 'Inicio de carpeta', value: 'fechaRegistroExpediente',  },
                    { text: '¿Desde cuando se derivo?', value: 'fechaDerivacion',  },
                    { text: '¿Hay alguna sesion satisfactoria?', value: 'statusSesion',   }, 
                    { text: '¿Hay algun acuerdo reparatorio?', value: 'idAcuerdoReparatorio', }, 
                    { text: '¿Se esta cumpliendo el acuerdo?', value: 'statusPago',  }, 
                    { text: 'Módulo', value: 'statusGeneral',  }, 
        ...
        created () {
          ...
          { 
            ...
            //Apis que se lanzan al m omento de abrir esta seccion
            me.listar();
    ...
    drawer(){
      return this.$store.state.drawer
        ...
        methods:{  
            ...
            //Nueva api para el listado de expedientes con rezago, vease la explicacion en BackEnd
            listar()
            {
            ...
            //Nuevas funciones}
            //Funcion para calcular los dias desde que se creo la carpeta de investigacion
            inicioCarpeta: function(fecha){

                //Variables para el calculo de dias
                var fechaActual = new Date();
                var fechaIngreso = new Date(fecha);
                //Calculo de los dias
                var diferenciaEnMilisegundos = fechaActual - fechaIngreso;
                //Esto para mostrar un valor en dias enteros
                var diasPasados = Math.floor(diferenciaEnMilisegundos / (1000 * 60 * 60 * 24));
                
                var texto =  moment(fecha).format('DD/MM/YYYY');

                if(texto == 'Fecha inválida') 
                {
                    var mensajeTabla = "Sin registro del inicio";
                }
                else
                {
                    var mensajeTabla = "Hace " +  diasPasados + " dias desde el " + texto;
                }
                return mensajeTabla;
            },
            //Funcion para calcular los dias desde que se derivo la carpeta a justicia restaurativa
            inicioDerivacion: function(fecha){
            ...
            //Funcion para calcular los dias desde que se llevo a cabo la ultima sesion satisfactoria o si acaso se llevo a cabo una sesion
            haySesionSatisfactoria: function(sesion, fecha){
                ...
                if(sesion == "Se realiza sesión con acuerdo reparatorio") 
                {
                    var mensajeTabla = "Si, desde hace " + diasPasados + " dias";
                }
                else
                {
                    var mensajeTabla = "No";
            ...
            //Funcion para calcular los dias desde que se creo la carpeta de investigacion o si acaso se creo el acuerdo reparatorio
            haySesionAcuerdoReparatorio: function(acuerdo, fecha){
                ...
                if(acuerdo != "00000000-0000-0000-0000-000000000000") 
                {
                    var mensajeTabla = "Si, desde hace " + diasPasados + " dias";
                }
                else
            ...
            //Funcion para calcular los dias desde que se cumplio los pagos o si acaso se no se cumplio alguno
            hayCumplimientoAcuerdo: function(seguimiento, fecha){
                ...
                if(seguimiento == "No pagado" || seguimiento == null) 
                {
                    var mensajeTabla = "No, se ha incumplido un pago";
                }
                else
                {
                    var mensajeTabla = "Si, desde hace " + diasPasados + " dias";
