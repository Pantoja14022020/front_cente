                                        ...
                                        <v-layout row wrap>
                                                ...
                                                <v-card elevation="0">
                                                    <v-card-text>
                                                        <table>
                                                            <tr>
                                                                <td>Conclusión:</td>
                                                                <td><b> La conclusión solo podra ser vista por el perito que la esta llenando.</b></td>
                                                            </tr>
                                                            <br>
                                                            <tr>
                                            ...
                                            <v-flex xs12 sm12 md12 lg12>
                                                <v-card elevation="0" v-if="changestatus.value == 1">
                                                    <v-card-title class="accent"><h3>Conclusión</h3></v-card-title>
                                                    <v-card-text>
                                                        <vue-editor
                                                            name="conclusíon"
                                                            v-model="conclusion"
            ...
            >
                <template slot="items" class="white" slot-scope="props">
                    ...
                    <td class="d-flex align-center">
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                                <v-icon class="mr-2" v-on="on" color = "warning" @click="imprimirdocs(props.item)">
                                    print
                                </v-icon>
                            </template>
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                                <v-icon class="mr-2" v-on="on" @click="obtenerdireccionhecho(props.item)">
                                    location_on
                                </v-icon>
                            </template>
                        ...
                        <div class="justify-center layout px-0" v-if="props.item.status != 'Finalizado' && props.item.status != 'Entregado' ">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-icon class="mr-2" v-on="on" @click="dialogostatus(props.item)">
                                        edit
                                    </v-icon>
                                </template>
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                                <v-icon class="mr-2" v-on="on" @click="consultcsmsj(props.item)">
                                    line_weight
                                </v-icon>
                            </template>
                            <span>Consultar Conclusión y mensaje</span>
                        </v-tooltip>
                        </div>
                        <div class="justify-center layout px-0" v-if="props.item.status != 'Asignado' && props.item.status != 'Enproceso' && props.item.status != 'Pospuesto'">
                        <v-tooltip bottom>
            ...
            <v-dialog v-model="dialogcsmsj" max-width="1000px">
                <v-card>
                    <v-toolbar card dark color="grey lighten-4 primary--text">
                        ...
                        <v-toolbar-title class="subheading">Conclusión y Mensaje</v-toolbar-title>
                        <v-spacer />
                    </v-toolbar>
                    <v-card-text>
                        <v-form ref="form" >
  ...
  export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            { text: 'Servicio', value: 'dServicio', sortable: true },
    ...
    methods:{
      ...
      handleFilesUpload(){
        ...
        onFilePickedfirma (e) {
            //var tempo = URL.createObjectURL(e.target.files[0]);
            var tempo;
			const files = e.target.files
            if (files[0] !== undefined) {
                this.archivoName = files[0].name;
                if (this.archivoName.lastIndexOf('.') <= 0) {
                    return
                }
                const fr = new FileReader ()
                fr.readAsDataURL(files[0])
                fr.addEventListener('load', () => {
                    this.archivoUrl = fr.result
                    this.archivoFile = files[0] // this is an file that can be sent to server...
        ...
        listarstatus(){
                ...
                me.$CAT.get('api/RDiligencias/FiltrarStatus/'+ me.numnuc.value,configuracion).then(function(response){
                    statusArray=response.data;
                    statusArray.map(function(x){
                        me.servf.push({text: x.servicio,value:x.idRDiligencias});
                    });
        ...
        descargaPdf (firma, laimagen, urlfirma, rfcid) {
            var pdfMake = require('pdfmake/build/pdfmake.js');
            var htmlToPdfmake = require("html-to-pdfmake");
            //
            if (pdfMake.vfs == undefined){
            ...
            //***************************************************************************** */
            // var html =htmlToPdfmake(
            //     '<div class="container"><vue-qr text="https:\\procuraduria.hidalgo.gob.mx" :size="200"></vue-qr></div>'
            //     //'Aqui texto html'
            // );
            ...
            //<embed src="https://localhost:44336/Carpetas/C2-2021-03019/Telmex.pdf" type="application/pdf" width="100%" height="600px" />
            //***************************************************************************** */
            var agenciainfodr = this.agenciainfo["direccion"];
            var agenciainfotel = this.agenciainfo["telefono"];
            //***************************************************************************** */
            var dd = {
                ...
                // //{ qr: urlfirma },
                // // colored QR
                // { qr: urlfirma, foreground: 'red', background: 'yellow' },
                // // resized QR
                // { qr: 'text in QR', fit: '100' },
                // ],
                ...
                content: [
                    {
                        ...
                        margin: [8, 650, 0, 0],
                        alignment: 'justify',
                        //qr: urlfirma, foreground: 'red', background: 'yellow', fit: 100,
                    },
                ],
...
div.v-list.theme--light  {
  max-width: 960px !important; /* Ajusta el ancho máximo según tu diseño */
}
