    ...
    <n403 v-if="e403" />
      <v-flex v-if="showpage">
        <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Bandeja de Entrada Solicitudes Sin Registro </v-toolbar-title>
    ...
    methods:{ 
        ...
        listar(){
          ...
          let configuracion= {headers : header};
                me.$PI.get('api/CMedicoPSR/ListarporModulo/'+me.u_idmoduloservicio,configuracion).then(function(response){
        ...
        activardialogstatus(item){
            let me = this;
            me.idcomparencia = item.idCMedicoPSR;
            me.dialog = true;
            me.limpiar();
        },
        actualizarstatus(){
            ...
            if (result) {
                if(me.status.value != 4){
                    me.$PI.put('api/CMedicoPSR/ActualizarStatus',{
                    'IdCMedicoPSR': me.idcomparencia,
                ...
                me.$PI.put('api/CMedicoPSR/ActualizarStatusFinalizado',{
                    'IdCMedicoPSR': me.idcomparencia,
                    'Status': me.status.text                  
                },configuracion).then(function(response){                            
                    me.$notify('La información se actualizo correctamente !!!','success')
