    ...
    methods :{
        ingresar(){
                this.$validator.validate().then(result => {
                    if (result) { 
                        this.error=null;
                        this.$controlacceso.post('api/Usuarios/Login', {usuario: this.usuario, password: this.password,  claveP: '536ec2a1-8c23-4f84-87ce-7d28f505950e'})
                        .then(respuesta => {
        ...
        tiempoSesion(){
          ...
          configurarTemporizadorCierreSesion() {
            ...
            const minutosEnMilisegundos =  this.avisoSesion * 60 * 1000; // 25 minutos en milisegundos
            const cierreSesion = this.sesionConcluida * 60 * 1000; // 30 minutos en milisegundos
            ...
            // Temporizador de notificación después de 30 minutos
                this.notificacionT = setTimeout(() => {
                  Swal.fire({
                              ...
                              }, minutosEnMilisegundos);  //30 minutos en milisegundos par mostrar notificacion   
                          ...
                          // Temporizador para cerrar la sesión 5 minutos después de la notificación
                              ...
                              }, cierreSesion); // 35min para cerrar sesion                          
            },
          CierreNav(){
