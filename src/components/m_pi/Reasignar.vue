    ...
    <v-list-tile
        ...
        <v-flex v-if="showpage">
            <v-toolbar flat color="white">
                <v-toolbar-title class="font-weight-regular" >Reasignacíon de unidad policial</v-toolbar-title>
            ...
            >
                <template slot="items" class="white" slot-scope="props">
                    ...
                    <td class="justify-center layout px-0">
                        <div  class="justify-center layout px-0" v-if="props.item.statusAC == 'Asignado' ||  props.item.statusAC == 'Enproceso' ">
                            <v-tooltip bottom>
                                <template v-slot:activator="{ on }">
                                    <v-icon class="mr-2" v-on="on" @click="listarModulos(props.item)">
                                    edit
                                    </v-icon> 
                                </template>
                                <span>Reazignar</span>
                            ...
                            <v-container grid-list-md>
                                <v-layout wrap>
                                    <v-flex xs12 sm12 md12>
                                        ...
                                        />
                                        <v-text-field 
                                            name="motivo de reazignación"
                                            label="*Motivo de reazignación:" 
                                            v-model='motivoreazignacion' 
                                            v-validate="'required'" 
                                            :error-messages="errors.collect('motivo de reazignación')"
                                        />
                                    </v-flex>                           
                                </v-layout>
                            </v-container>
    ...
    export default {
        ...
        data: () => ({
            ...
            headers: [
                ...
                { text: 'Unidad policial', value: 'modulo' }, 
        ...
        computed: {
            formTitle () {
                return this.editedIndex === -1 ? 'Reazignar unidad policial' : 'Razón Rechazado'
            },
            formIcon () {
        ...
        methods:{
            ...
            listarModulos (item) {
                ...
                me.$conf.get('api/ModuloServicios/ListarPorDistritoidUsario/' + me.u_iddsp, configuracion).then(function(response) {
                    ...
                    me.idPeritoAsignado  = item.idPeritoAsignadoPI;
                    me.editedIndex = -1;
                    me.dialog = true;
                }).catch(err => { 
                    if (err.response.status == 400) {
