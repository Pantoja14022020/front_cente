        ...
        listarporfecha(){
          ...
          me.$validator.validate().then(result => {     
            if (result) {   
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                        axios.get('api/PeritoAsignadoPI/Listarporidfecha/'+ me.fechaInicio +" 00:00:00"+'/'+me.fechaFin+" 23:59:59",configuracion).then(function(response){
...
/////////////////////////////////////////////////////////////////////////////
                        ...
                        for(var j = 0;j< me.registros.length;j++){
                            if(me.registros[j].mes == registro.ultmimoStatus.substring(8,10) )  {  
                                estado = true;                            
                                if(registro.statusAC == 'Finalizado')
                        ...
                        if(!estado) { 
                            ...
                            estado = false;
                                if(registro.statusAC == 'Finalizado')
                                    me.registros.push({mes: registro.ultmimoStatus.substring(8,10), asignado:0,enproceso:0,finalizado:1,suspendido:0,pospuesto:0,entregado:0,total:0});
                                else if(registro.statusAC =='Asignado')
                                    me.registros.push({mes: registro.ultmimoStatus.substring(8,10), asignado:1,enproceso:0,finalizado:0,suspendido:0,pospuesto:0,entregado:0,total:0});
                                else if(registro.statusAC == 'Enproceso')
                                    me.registros.push({mes: registro.ultmimoStatus.substring(8,10), asignado:0,enproceso:1,finalizado:0,suspendido:0,pospuesto:0,entregado:0,total:0});
                                else if(registro.statusAC == 'Suspendido')
                                    me.registros.push({mes: registro.ultmimoStatus.substring(8,10), asignado:0,enproceso:0,finalizado:0,suspendido:1,pospuesto:0,entregado:0,total:0});
                                else if(registro.statusAC == 'Pospuesto')
                                    me.registros.push({mes: registro.ultmimoStatus.substring(8,10), asignado:0,enproceso:0,finalizado:0,suspendido:0,pospuesto:1,entregado:0,total:0});
                                else if(registro.statusAC == 'Entregado')                              
                                    me.registros.push({mes: registro.ultmimoStatus.substring(8,10), asignado:0,enproceso:0,finalizado:0,suspendido:0,pospuesto:0,entregado:1,total:0});      
