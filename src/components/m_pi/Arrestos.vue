    ...
    <n403 v-if="e403" />
      <v-flex v-if="showpage">
        <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Arrestos</v-toolbar-title>
        ...
        <v-data-table
                ...
                <template slot="items" class="white" slot-scope="props">
                    ...
                    <td>{{ props.item.tiempoH +" hrs" }}</td> 
                    <td>{{ props.item.juezSolicita }}</td>                     
        ...
        <v-dialog v-model="dialogo" fullscreen hide-overlay transition="dialog-bottom-transition">
            ...
            <v-toolbar dark color="primary">
                ...
                <v-toolbar-title>Registrar arresto</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-toolbar-items>
            ...
            <v-container grid-list-md text-xs-center>
                <v-layout row wrap>
                    <v-flex class="espaciado" xs12 sm12 md6 lg6 >
                        ...
                        <v-text-field 
                            name="nombre del arrestado"
                            label="*Nombre del arrestado:" 
                            v-model='nombrearrestado' 
                            v-validate="'required'" 
                            :error-messages="errors.collect('nombrearrestado')"
                        ></v-text-field>
                        ...
                        <v-menu
                            ...
                            :return-value.sync="fecharecibido"
                        ...
                        >
                            ...
                            ></v-text-field>
                                ...
                                <v-date-picker locale="es" v-model="fecharecibido" no-title scrollable>
                                <v-spacer></v-spacer>
                    ...
                    <v-flex class="espaciado" xs12 sm12 md6 lg6 >
                        ...
                        <v-text-field 
                            name="juez que lo solicita"
                            label="*Juez que lo solicita:" 
                            v-model='juezsolicita' 
                            v-validate="'required'" 
                            :error-messages="errors.collect('juez que lo solicita')"
                        ></v-text-field>
                        ...
                        <v-text-field 
                            ...
                            v-validate="'required'" 
                            :error-messages="errors.collect('grupo asignado')"
                        ></v-text-field>
  ...
  export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            { text: 'Tiempo', value: 'tiempoH' },
            { text: 'Juez que solicita', value: 'juezSolicita' },
    ...
    methods:{ 
        ...
        guardar(){
           this.$validator.validate().then(result => {
                    if (result) { 
                            ...
                            me.$PI.post('api/Arresto/Crear',{  
                                'ModuloServicioId' :me.u_idmoduloservicio,
                                'FechaRecibido'  :me.fechar,
                                'Oficio' :me.oficio,
                                'CausaPenal' :me.causapenal.text,
                                'NomArrestado' :me.nombrearrestado,
                                'TiempoH' :me.tiempo,
                                'JuezSolicita':me.juezsolicita,
                                'GpoAsignado' :me.grupoasignado,
                                'Status' :"Solicitado",
                                'Estado' :"Creado",
                                'Respuesta' :"",
        ...
        fechainif(){
            if(this.menu1){
                this.generarfecha2();
            this.$refs.menu1.save(this.fecharecibido);
            }  
            if(this.menu2){
                this.generarfecha2();
            this.$refs.menu2.save(this.fechacomparecencia);
            }   
            if(this.menu3){
                this.generarfecha2();
        ...
        generarfecha2(){           
            if(this.menu1) 
            this.fechar =this.fecharecibido.substring(8,10) +" de "+ this.obtenermes(this.fecharecibido.substring(5,7)-1)+
            " del "+ this.fecharecibido.substring(0,4);   
        ...
        listar(){
          ...
          let header={"Authorization" : "Bearer " + this.$store.state.token};
          let configuracion= {headers : header};
          me.$PI.get('api/Arresto/Listar/'+ me.u_idmoduloservicio,configuracion).then(function(response){
                    //console.log(response);
                    me.arrestos=response.data;
                }).catch(err => { 
                    if (err.response.status==400){
                        me.$notify("No es un usuario válido", 'error')
                    } else if (err.response.status==401){
