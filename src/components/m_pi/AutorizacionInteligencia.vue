    ...
    <n403 v-if="e403" />
      <v-flex v-if="showpage">
        <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Autorización de solicitudes de inteligencia</v-toolbar-title>
        ...
        <v-data-table
                ...
                <template slot="items" class="white" slot-scope="props">
                    ...
                    <td>{{ props.item.statusMensaje }}</td> 
                    <td class="justify-center layout px-0" >
                        <v-tooltip bottom >
                            <template v-slot:activator="{ on }">
                                <v-icon 
                                    class="mr-2" v-on="on" 
                                    @click="vermensaje(props.item)"
                                    >
                                visibility
                                </v-icon> 
                            </template>
                            <span>Ver mensaje de solicitud</span>
                        </v-tooltip>
                        <div class="justify-center layout px-0" v-if="props.item.statusMensaje == 'Solicitado'" >
                            <v-tooltip bottom   >
                                <template v-slot:activator="{ on }">
                                    <v-icon 
                                        class="mr-2" v-on="on" 
                                        @click="aprobarsoli(props.item)"
                                        >
                                    done
                                    </v-icon> 
                                </template>
        ...
        <v-dialog  v-model="dialogomensaje"  max-width="1000px" >
              
            <v-card>
                <v-toolbar card dark color="grey lighten-4 primary--text">
                    <v-avatar  size="30">
                        <v-icon class="grey lighten-2">chat</v-icon>
                    </v-avatar>
                    
                         
                    <v-toolbar-title class="subheading">Solicitud</v-toolbar-title>
                    <v-spacer></v-spacer>  
                    ...
                    <v-card-text>
                        <v-form ref="form" >
                                ...
                                <v-layout row wrap>
                                    <v-flex xs12 sm12 md12 lg12>
                                        <v-card auto-grow elevation="0">                                
                                            <v-card-text >
                                                <v-list three-line   > 
                                                    <p class="text-lg-center font-weight-bold"><a>Mensaje de solicitud</a></p> 
                                                    <v-list-tile>  
                                                        <v-list-tile-action>
                                                            <v-icon color="success">subject</v-icon>
                                                        </v-list-tile-action> 
                                                        <v-list-tile-content   >  
                                                            <v-list-tile-title> <p class="body-4 font-weight-bold"><a>Mensaje:</a></p> </v-list-tile-title> 
                                                            <v-list-tile-sub-title   > <p class="caption font-weight-regular" v-html="mensajeso" ><a>{{ mensajeso }}</a></p>  </v-list-tile-sub-title>  
                                                        </v-list-tile-content> 
                                                    </v-list-tile>                           
                                                </v-list>  
                                            </v-card-text> 
                            ...
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn @click="dialogomensaje=false" >Cerrar</v-btn>                             
                            </v-card-actions> 
                        </v-form>
        ...
        <v-dialog  v-model="dialogoaprobar"  max-width="1000px" >
                ...
                <v-toolbar card dark color="grey lighten-4 primary--text">
                    ...
                    <v-card-text>
                        <v-form ref="form" data-vv-scope="aceptar" >
                            ...
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn @click="dialogoaprobar=false" >Cerrar</v-btn>   
                                <v-btn color="success" @click="aprobarsolicitud()" >Asignar</v-btn>                          
                            </v-card-actions> 
                        </v-form>
        ...
        <v-dialog  v-model="dialogorechazar"  max-width="1000px" >
            ...
            <v-card>
                <v-toolbar card dark color="grey lighten-4 primary--text">
                    ...
                    <v-toolbar-title class="subheading">Rechazar Solicitud</v-toolbar-title>
                    <v-spacer></v-spacer>  
                        ...
                        <v-form ref="form" data-vv-scope="rechazar" >
                            ...
                            <v-container grid-list-md text-xs-center>
                                <v-layout row wrap>
                                    <v-flex xs12 sm12 md12 lg12>
                                        <v-card elevation="0"  >
                                            <v-card-title class="accent"><h3>*Motivo de rechazo</h3></v-card-title>
                                                <v-card-text>
                                                    <vue-editor  
                                                        ...
                                                        v-validate="'required'"  
                                                        data-vv-scope="aceptar"                                    
                                                        :error-messages="errors.collect('rechazar.razón rechazado')">
                                                    </vue-editor>
                                                </v-card-text>   
                                        </v-card>                                       
                                    </v-flex>
                            ...
                            <v-card-actions>
                                <v-spacer></v-spacer>                     
                                <v-btn @click="dialogorechazar=false" >Cerrar</v-btn>
                                <v-btn color="success" @click="rechazarsolicitud()" >Rechazar</v-btn>                             
                            </v-card-actions> 
                        </v-form>
  ...
  export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            { text: 'Fecha de Generación', value: 'fecha' }, 
            { text: 'Status',value: 'statusMensaje' }, 
    ...
    methods:{ 
        ...
        vermensaje(item){
            let me = this;
            me.dialogomensaje = true;
            me.mensajeso = item.mensaje;
        },  
        aprobarsoli(item){
            let me =this;
            me.limpiar();
            me.dialogoaprobar = true;
            me.solicitudInteligenciaId = item.idSolicitudInteligencia
        },
        rechazarsoli(item){
            let me =this;
            me.limpiar();
            me.dialogorechazar = true;
            me.solicitudInteligenciaId = item.idSolicitudInteligencia
        },
        listarModulos(item){ 
            var arraymodulos = [];
            ...
            me.$conf.get('api/ModuloServicios/ListarPorDistritoServicioIn/'+ me.u_iddsp, configuracion).then(function(response){
        ...
        aprobarsolicitud(){
            this.$validator.validateAll('rechazar').then(result => {
                    if (result) {
                            let me=this;                            
                            let header={"Authorization" : "Bearer " + this.$store.state.token};
                            let configuracion= {headers : header};                          
