    ...
    <n403 v-if="e403" />
      <v-flex v-if="showpage">
        <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Lista de detenidos</v-toolbar-title>
                    ...
                    <td class="justify-center layout px-0" > 
                        <v-tooltip bottom  >
                            <template v-slot:activator="{ on }">
                                <v-icon 
                                    class="mr-2" v-on="on"  
                                    @click="consultarvisitas(props.item)"
                                    >
                                    import_contacts
                                </v-icon> 
                            </template>
                            <span>Ver historial de personas visitadas</span>
                        </v-tooltip>  
                    </td>                  
                </template>
         ...
         <v-dialog  v-model="dialogo"  max-width="1000px" >
            <v-card>
                <v-toolbar card dark color="grey lighten-4 primary--text">
                    ...
                    <v-toolbar-title class="subheading">Historial de personas visitadas </v-toolbar-title>
                    <v-spacer></v-spacer>
                    ...
                    <v-form ref="form" >
                        <v-container grid-list-md>
                            ...
                            <v-flex xs12 sm12 md12 lg12>
                                <v-text-field class="text-xs-center" v-model="searchvisitas" append-icon="search" label="Búsqueda" single-line hide-details></v-text-field>
                                <v-data-table                                             
                                    :headers="headersvisitas"
                                    :items="historialvisitas"
  ...
  export default {
    ...
    data: () => ({
        ...
        headersvisitas:[
            ...
            { text: 'Emitida por', value: 'usuario', sortable: true  }, 
            { text: 'Modulo', value: 'modulo', sortable: true  },             
            { text: 'Persona que visita', value: 'nombreVisita' },
            { text: 'Relación con detenido', value: 'rDetenido' },
            { text: 'Hora entrada', value: 'hILocutorio' },
            { text: 'Hora Salida',value: 'hSLocutorio' },    
    ...
    methods:{ 
        ...
        consultarvisitas(item){
          let me=this;  
          let header={"Authorization" : "Bearer " + this.$store.state.token};
          let configuracion= {headers : header};
          me.$PI.get('api/Visita/ListarporIddetenido/'+ item.idDetencion,configuracion).then(function(response){
