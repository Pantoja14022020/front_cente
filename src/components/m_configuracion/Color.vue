    ...
    <v-layout align-start>
        ...
        <v-flex v-if="showpage">
            <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Color de Vehiculos.</v-toolbar-title>
    ...
    export default {
        ...
        data(){
            return {
                ...
                dialog: false,
                headers: [
                    { text: 'Color', value: 'dato', sortable: true },
        ...
        computed: {
            formTitle () {
                return this.editedIndex === -1 ? 'Nuevo Color' : 'Actualizar color'
            },
              formIcon () {
        ...
        methods:{
            listar(){
                ...
                this.$conf.get('api/Color/Listar',configuracion).then(function(response){
                ...
                this.$validator.validate().then(result => {
                    if (result) {
                        if (this.editedIndex > -1) {
                            ...
                            let header={"Authorization" : "Bearer " + this.$store.state.token};
                            let configuracion= {headers : header};
                            this.$conf.put('api/Color/Actualizar',{
                                'idColor':me.idColor, 
                                'dato': me.color
                            ...
                            let header={"Authorization" : "Bearer " + this.$store.state.token};
                            let configuracion= {headers : header};
                            this.$conf.post('api/Color/Crear',{
                                'dato': me.color, 
                            },configuracion).then(function(response){
                                me.close();
