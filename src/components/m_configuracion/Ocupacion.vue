    ...
    <v-layout align-start>
        ...
        <v-flex v-if="showpage">
            <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Ocupación.</v-toolbar-title>
...
<script>
    ...
    export default {
        ...
        computed: {
            formTitle () {
                return this.editedIndex === -1 ? 'Nueva ocupación' : 'Actualizar ocupación'
            },
              formIcon () {
        ...
        methods:{
            listar(){
                ...
                axios.get('api/Ocupacions/Listar',configuracion).then(function(response){
                    //console.log(response);
                    me.ocupacions=response.data;
                }).catch(err => { 
            ...
            editItem (item) {
                this.idOcupacion= item.idOcupacion;
                ...
                this.$validator.validate().then(result => {
                    if (result) { 
                        if (this.editedIndex > -1) {
                            ...
                            let header={"Authorization" : "Bearer " + this.$store.state.token};
                            let configuracion= {headers : header};
                            axios.put('api/Ocupacions/Actualizar',{
                            ...
                            let header={"Authorization" : "Bearer " + this.$store.state.token};
                            let configuracion= {headers : header};
                            axios.post('api/Ocupacions/Crear',{
                                'nombre': me.nombre, 
                            },configuracion).then(function(response){
                                me.close();
