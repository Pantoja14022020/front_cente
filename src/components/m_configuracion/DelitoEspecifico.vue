      ...
      <NavDrawer></NavDrawer>
        ...
        <v-flex v-if="showpage">
            <v-toolbar flat color="white">
                    <v-toolbar-title class="font-weight-regular" >Delito específico.</v-toolbar-title>
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                ...
                headers: [
                    ...
                    { text: 'Delito', value: 'nombreDelito' } ,
        ...
        computed: {
            formTitle () {
                return this.editedIndex === -1 ? 'Nuevo delito específico' : 'Actualizar delito específico'
            },
              formIcon () {
        ...
        methods:{
            ...
            editItem (item) {
                this.idDelitoEspecifico= item.idDelitoEspecifico; 
                this.nombre=item.nombre; 
                this.delito = new Object();
                this.delito.value = item.delitoId
                this.editedIndex=1;
                this.dialog = true
            },
            ...
            listarDelitos(){
                ...
                me.$conf.get('api/Delitoes/Listar',configuracion).then(function(response){
                    ...
                    delitosarray.map(function(x){
                        me.delitos.push({text: x.nombre,value: x.idDelito});
                    });
                }).catch(err => { 
                    if (err.response.status==400){
                        me.$notify("No es un usuario válido", 'error')
