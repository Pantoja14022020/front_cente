    ...
    <v-flex v-if="showpage" id="mapa">
      <v-toolbar flat color="white">
        <v-toolbar-title class="font-weight-regular"
          >Servicios periciales.</v-toolbar-title
        >
      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td class="layout">
            ...
            <div
              class="layout"
              v-if="props.item.statusRespuesta != 'Solicitado'"
            >
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
              ...
              <v-layout row wrap>
                <v-flex class="espaciado" xs12 sm12 md6 lg6>
                  ...
                  <v-autocomplete
                    ...
                    :error-messages="errors.collect('sp')"
                  />
                  <v-autocomplete
                    name="agencia"
                    :items="agencias"
                  ...
                  <v-switch
                    color="success"
                    v-model="conindicio"
                    :label="`¿El servicio pericial contiene indicio ?: ${textoswitch}`"
                  />
                  <v-select
                    v-if="conindicio"
                    name="indicio"
                    ...
                    multiple
                    v-validate="'required'"
                    :error-messages="errors.collect('indicio')"
                  />
                  <v-switch
...
export default {
  ...
  methods: {
    ...
    vermsj(item) {
      if (item.statusRespuesta == "Rechazado") {
    ...
    listarFiltroSP() {
      ...
      me.serviciopericialeId.map(function (x) {
        array.push({ value: x.value });
        me.$conf
          .get(
            "api/ASPs/FiltrarStatusSP/" + x.value + "/" + me.agencia.value,
            configuracion
          )
          .then(function (response) {
            ...
            me.atencionVictimas = response.data.atencionVictimas;
            me.obtenernumeromaximo();
            nombre.push(me.nombreServicio);
            me.newNombreservicio = nombre;
            if (me.listaMateriaConcat == "") coma = "";
            else coma = ", ";
            // CONCATENAR LA LISTA DE MATERIAS PARA EL TEXTO FINAL
            me.listaMateriaConcat =
              me.listaMateriaConcat + coma + response.data.materia;
            //CONCATENAR LOS SERVICIOS PARA EL TEXTO PRINCIPAL
            me.nombreServicioConcat =
              me.nombreServicioConcat + coma + response.data.nombreServicio;
            console.log("nombreConcat en " + i + ":" + me.nombreServicioConcat);
            //
            ASPids.push(me.aspid);
      ...
      me.$conf
        ...
        .then(function (response) {
          ...
          sparrays.map(function (x) {
            const usuarioAgencia = "División Científica - FEDC";
            const usuarioAgencia3 =
              "División Científica de la Agencia de Investigación Criminal";

            if (me.u_idagencia === "9b600cdb-5c61-4392-a16b-e0dd93412da1") {
              if (x.dspn === usuarioAgencia || x.dspn === usuarioAgencia3) {
                me.sps.push({ text: x.dspn, value: x.dspId });
              }
            } else if (
              me.u_idagencia === "f7243ac2-c5cd-4e99-a959-4bef65e82247"
            ) {
              const usuarioAgencia1 = "División Científica - UECS";
              const agenciaSeleccionada1 = sparrays.find(
                (x) => x.dspn === usuarioAgencia1
              );
              if (agenciaSeleccionada1) {
                me.sps.push({
                  text: agenciaSeleccionada1.dspn,
                  value: agenciaSeleccionada1.dspId,
                });
              }
            } else if (
              me.u_idagencia === "40d56198-80c0-4379-a4a6-6b19bf0360c1"
            ) {
              const usuarioAgencia2 = "División Científica - FDT";
              const agenciaSeleccionada2 = sparrays.find(
                (x) => x.dspn === usuarioAgencia2
              );
              if (agenciaSeleccionada2) {
                me.sps.push({
                  text: agenciaSeleccionada2.dspn,
                  value: agenciaSeleccionada2.dspId,
                });
              }
            } else {
              const agenciaSeleccionada3 = sparrays.find(
                (x) => x.dspn === usuarioAgencia3
              );
              if (agenciaSeleccionada3) {
                me.sps.push({
                  text: agenciaSeleccionada3.dspn,
                  value: agenciaSeleccionada3.dspId,
                });
              }
            }
          });
    ...
    imprimirdiligencia(item) {
      ...
      me.generarQR(me.docDiligencia,me.nuc,item.emitidoPor,item.fechaSys,item.idRDiligencias);
      this.modaldocumento = false;

      setTimeout(() => 
      {
        this.imprimirDiligencia(item.emitidoPor, item.uPuesto, item.agencia);
    ...
    listarTL() {
      ...
      me.$validator.validate().then((result) => {
        console.log("validado el formulario");
        console.log(result);
        if (result) {
          me.botonvistaprev = false;
          me.nombre = me.personaId.text;
          if (me.statusdirigido) persona = me.nombre;
          else persona = "PERSONA";

          me.textolibre =
            "<p>Por medio de la presente, <strong>" +
            me.listaMateriaConcat +
          ...
          console.log("error de validaciòn");
        }
      });
    },
    listarrHecho() {
    ...
    imprimirSP() {
      ...
      var now = moment();
      me.generarQR(me.docDiligencia,me.nuc,me.u_nombre,now,me.idrdiligencias);
    ...
    crearPDF_SP() {
      ...
      var descripcionRegTabI =
        "Registro de solicitud de servicios periciales a " +
        me.sp.text +
      ...
      if (me.editedIndex == 1) {
        ...
        console.log(me.serviciopericialeId.length);
        me.$confirm(
            ...
            for (var i = 0; i < me.serviciopericialeId.length; i++) {
              me.$CAT
                ...
                .then(function (response) {
                  ...
                  var rDiligenciasId =  response.data.iddili;

                  var now = moment();
                  me.vistaPreviaTF = false;
                  me.generarQR(me.docDiligencia,me.nuc,me.u_nombre,now,rDiligenciasId);

                  if (me.conindicio) {
                    for (var j = 0; j < me.indicio.length; j++) {
                      me.$SP
                        .post(
                          "api/DiligenciaIndicio/Crear",
                          {
                            rDiligenciasId: rDiligenciasId,
                            indiciosId: me.indicio[j].value,
                          },
                          configuracion
                        )
                        .then(function (response) {
                  ...
                  setTimeout(() => 
                  {
                    me.imprimirDiligencia(me.u_nombre, me.u_puesto, me.u_agencia);
    ...
    imprimirDiligencia(nombre, puesto, agencia) 
    {
      let me = this;
