let axios = require('axios').default;
// COPIAR CARPETA
export const copiarCarpeta = (configuracion,servicios,nombreServicios,posicionServicio,idNUC,idHecho,idAtencion,distrito,idModulo,idAgencia,idRemision ) => 
{
    return new Promise((resolve, reject)=>
    {
        this.$cat.post(servicios[posicionServicio],
        {  
            'IdDistrito': distrito,
            'IdRAtencion': idAtencion,
            'IdNuc':idNUC,
            'IdRHecho':idHecho

        },configuracion).then(function(response)
        {
            posicionServicio++;

            console.log("Posicion:"+posicionServicio);
            console.log("tamaño array:"+servicios.length);

            if(posicionServicio<servicios.length)
            {
                copiarCarpeta(configuracion,servicios,nombreServicios,posicionServicio,idNUC,idHecho,idAtencion,distrito).then(function(response)
                {
                    resolve();

                }).catch(err=>{
                    if (err.response.status==400){
                        me.$notify("No es un usuario válido", 'error')
                    } else if (err.response.status==401){
                        me.$notify("Por favor inicie sesion para poder navegar en la aplicacion", 'error')
                        me.e401 = true,
                        me.showpage= false
                    } else if (err.response.status==403){ 
                        me.$notify("No esta autorizado para ver esta pagina", 'error')
                        me.e403= true
                        me.showpage= false 
                    } else if (err.response.status==404){
                        me.$notify("El recuso no ha sido encontrado", 'error')
                    }else{
                        me.$notify('Error al intentar crear el  registro!!!','error')  
                    }
                    reject(); 
                });
            }
            else
            {
                resolve();
            }
        
        }).catch(err => 
        {
            // Realizar la segunda petición (PUT) aquí después de manejar el error
            this.$cat.put('api/RHechoes/ActualizarModuloyAgencia',
            {
                'IdRHecho': idHecho,
                'moduloServicioId': idModulo,
                'agenciaId': idAgencia

            }, configuracion).then(function(response) 
            {
                if(idRemision != '00000000-0000-0000-0000-000000000000')
                {
                    this.$cat.put('api/RemisionesUI/ActualizarEnvioExitoso',
                    {
                        'IdRemisionUI': idRemision,
                        'EnvioExitosoTF': 0,

                    }, configuracion).then(function(response) 
                    {
                        resolve();

                    }).catch(err => 
                    {
                        resolve();
                    });

                }
                else
                {
                    resolve();
                }

            

            }).catch(err => {
                // Manejar los errores de la segunda petición si es necesario
            });
        

        });   
    });   
}