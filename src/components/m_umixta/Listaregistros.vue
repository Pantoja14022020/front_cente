<template>
    <v-layout align-start>
        ...
        <n403 v-if="e403" />
            ...
            <v-data-table
                ...
                <template slot="items" class="white" slot-scope="props">
                    ...
                    <td class="justify-center layout px-0">
                        ...
                        <v-tooltip bottom   >
                                <template v-slot:activator="{ on }">
                                    <v-icon  class="mr-2" color="warning"  v-on="on" @click="editItem(props.item)">
                                    folder
                                    </v-icon>
                                </template>
                    ...
                    <td>
                        <div v-if="props.item.statusAtencion">
                    ...
                    <td>
                        <div v-if="props.item.statusRegistro">
                    ...
                    <td>
                        <div v-if="props.item.contencionVicitma ">
                            <v-icon   class="mr-2" color="error">lens</v-icon>
                        </div>
                        <div v-else>
                            <v-icon   class="mr-2" color="success">panorama_fish_eye</v-icon>
                        </div> 
                    </td>  
        ...
        <v-dialog v-model="modalDetalleInformacion" fullscreen hide-overlay transition="dialog-bottom-transition">
    
            <v-card>
            <v-toolbar dark color="primary">
            
                <v-toolbar-title>Información del Registro de Atención Ciudadana.</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-toolbar-items>
                <v-btn  color=success text >
                {{ rac}}</v-btn>
                <v-btn icon   @click="modalDetalleInformacion = false">
                <v-icon>close</v-icon>
                </v-btn>
                </v-toolbar-items>
            </v-toolbar>
                        ...
                        <v-layout row wrap justify-center>
                            <v-flex  xs12 md12 lg12> 
                                <v-card  flat color="white" grid-list-md text-md-right fluid fill-height> 
                                    ...
                                    <v-layout row >
                                        <v-card elevation="0"   >
                                            ...
                                            <v-card-text>
                                                <table>
                                                        <tr>
                                                            <td style="width:180px"> Vicitma y/u ofendido: </td>
                                                            <td><b> {{nombre}}</b></td> 
                                                        </tr>
                                                        <tr>
                                                            <td>Tipo de vicitma:</td>
                                                            <td><b> {{ tvictima}}</b></td>
                                                        </tr>
                                                        <tr>
                                                ...
                                                <table>
                                                    ...
                                                    <table>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style="color:#78BE20" ><b ><v-icon small>access_time</v-icon> {{fechaAtencion.substring(8,10) +" de "+ obtenermes(fechaAtencion.substring(5,7)-1)+" del "+fechaAtencion.substring(0,4)+", "+fechaAtencion.substring(11,19)   }} </b></td>
                                                        </tr>
                                                        <tr>
                                                            <td >Imprimir  caratula de RAC</td>
                                                            <td> <v-btn text icon @click="mostrarpdf_CaratulaRAC"  >
                                                                <v-icon  >print</v-icon>
                                                            </v-btn> </td>
                                                        </tr>
                                    ...
                                    <v-layout row >
                                        <v-card elevation="0">
                                            <v-card-title class="accent" ><h3>Servicios para la atención a victima</h3></v-card-title>
                                            <v-card-text>
                                                <v-data-table
                                                ...
                                                <template slot="no-data">
                                                <v-btn color="primary" @click="listardiligencias()"  >Resetear</v-btn>
                                                </template>
                                                </v-data-table>
                                        ...
                                        <v-card elevation="0" >
                                            <v-card-title  class="accent"><h3>Oficios de derivación</h3></v-card-title>
                                            <v-card-text>
                                                <v-data-table
        ...
        <v-dialog v-model="modal_CaratulaRAC" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                ...
                <v-card-text>
                     <iframe id="iframepdf1" type="application/pdf" width="100%" height="835" frameborder="0" scrolling="no"  ></iframe>
                </v-card-text>
            </v-card>
        </v-dialog>

        <v-dialog v-model="modal_DependeciaDerivacion" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark color="primary">
                ...
                <v-spacer></v-spacer>
                <v-toolbar-items>
                <v-btn  color=success text @click.native="impirmiDependeciaDerivacion()">
                   IMPRIMIR</v-btn>
                  <v-btn icon   @click="modal_DependeciaDerivacion = false">
                <v-icon>close</v-icon>
                </v-btn>
                </v-toolbar-items>
            </v-toolbar>
    ...
    export default {
        ...
        methods:{
            ...
            editItem (item) { 
                ...
                this.rAtencionId  = item.idRAtencion;
                this.rac = item.rac;
            ...
            listarrHecho(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};   
                this.$cat.get('api/RHechoes/ListarTodosRegistros/' +me.rAtencionId,configuracion).then(function(response){
            ...
            listardiligencias(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header}; 
                this.$cat.get('api/RDiligencias/Listar/' + me.rHechoId,configuracion).then(function(response){
            ...
            imprimirdiligencia(item) { 
                ...
                this.subproc = item.uDirSubPro
                this.agenciai = item.agencia   
            ...
            listarDDerivacion(){
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header}; 
                this.$cat.get('api/RDDerivacions/Listar/' + me.rHechoId,configuracion).then(function(response){
                ...
                var dd = {
                    ...
                    content: [
                        ...
                        table: {
                            ...
                            widths: ['*', 'auto', 'auto'], 
                            body: [
                                [{colSpan: 3,  text:'DATOS DEL COMPARECIENTE', fillColor: '#eeeeee',style:'Titulo'},'', '', ],
                                [{style:'Texto',text:'Nombre' + '\n\n' + (this.nombre != "" ? this.nombre : "NA")},{style:'Texto',text: 'Alias:' + '\n\n' + (this.alias != "" ? this.alias : "NA") },{style:'Texto',text: 'Clasificación de persona:'+ '\n\n' + (this.tvictima != "" ? this.tvictima : "NA")}]  ,
                                [{style:'Texto',text:'Tipo persona:' + '\n\n' + (this.tipopersona != "" ? this.tipopersona : "NA")},{style:'Texto',text: 'CURP:'+ '\n\n' + (this.curp != "0" ? this.curp : "NA")},{style:'Texto',text: 'Documento de identificación:'+ '\n\n' + (this.docIdentificacion != "" ? this.docIdentificacion : "NA")}],
                                [{style:'Texto',text:'Nacionalidad:'+ '\n\n' + (this.nacionalidad != "" ? this.nacionalidad : "NA")},{style:'Texto',text: 'Teléfono(s):'+ '\n\n' + (this.tel != "" ? this.tel : "NA")},{style:'Texto',text: 'Correo electrónico:'+ '\n\n' + (this.email != "" ? this.email : "NA")}],
                                [{colSpan: 3,  text:'Dirección:'+ '\n\n' + this.direccion,style:'Texto'} , ''],
                                [{style:'Texto',text:'Género:' + '\n\n' + (this.genero != "" ? this.genero : "NA")},{style:'Texto',text: 'Sexo' + '\n\n' + (this.sexo != "" ? this.sexo : "NA")},{style:'Texto',text: 'Edad:' + '\n\n' + (this.edad != 0 ? this.edad : "NA")} ]  ,
                                [{style:'Texto',text:'Autoriza medio notificación:'+ '\n\n' + (this.medionotificacion != "" ? this.medionotificacion : "NA")},{style:'Texto',text: 'Ocupación:'+ '\n\n' + (this.ocupacion != "" ? this.ocupacion : "NA")},{style:'Texto',text: 'Nivel estudios:'+ '\n\n' + (this.nivelEstudio != "" ? this.nivelEstudio : "NA")}],
                                [{style:'Texto',text:'Religión:'+ '\n\n' + (this.religion != "" ? this.religion : "NA")},{style:'Texto',text: 'Idioma  o lengua que habla:'+ '\n\n' + (this.lengua != "" ? this.lengua : "NA")},{style:'Texto',text: 'Estado familiar:'+ '\n\n' + (this.estadocivil != "" ? this.estadocivil : "NA")}],  
                                [{colSpan: 3,  text:'Tiene alguna discapacidad:'+ '\n\n' +(this.tipodiscapacidad != "" ? this.tipodiscapacidad : "NA"),style:'Texto'} , ''], 
                            ]
                        },
                        layout: {
            ...
            crearPdf_DependeciaDerivacion() {
                ...
                var htmltexto = htmlToPdfmake( 
                 ...
                 var especificaciones = htmlToPdfmake( 
                    this.dd_espesificaciones
                );
                ...
                var dd = {
                    ...
                    content: [
                        ...
                        {
                            text:
                            this.dd_distrito+", Hidalgo a "+fecha+ "\n\n\n\n\n\n\n\n",                        
                            style: 'Fecha',
                            alignment: 'right', 
                        },
                        {
            ...
            downloadPdfdiligencia(nombre,puesto,agencia) {
                ...
                var logo1 = this.logo1;
                var logo2 = this.logo2;
                var u_dirSubPro = this.subproc;
                var u_agencia = this.agenciai;
                var nuc = this.nuc;
                //***************************************************************************** */
                ...
                var dd = {
                    ...
                    
                    {
                        ...
                        {
                            text:
                            this.distritoinicial + ", Hidalgo a "+this.fecha,                        
                            style: 'Fecha',
                            alignment: 'right', 
                        },    
                        {
