        ...
        <v-card>
                ...
                <v-card-text>
                    <v-form ref="form">
                        ...
                        <v-card-actions>
                            <v-spacer />
                            <v-btn @click="desactivardialogagencia()">Cancelar</v-btn>
                            <v-btn @click="filtrarAgenciaU()" class="success">Filtrar</v-btn>
                        </v-card-actions>
                    </v-form>
                </v-card-text>
         </v-card>
  ...
  export default {
    ...
    methods: {
      ...
      listar () {
        ...
        this.$cat.get('api/RegistroTableroI/ListarA/' + me.u_iddsp,configuracion).then(function(response) {
      ...
      filtrarAgenciaU () {
        ...
        let header = { "Authorization" : "Bearer " + this.$store.state.token }
        let configuracion = { headers : header }
        this.$cat.get('api/RegistroTableroI/ListarAEspecifico/' + me.u_iddsp + '/' + me.agenciau,configuracion).then(function(response) {
          me.carpetas=response.data;
          me.desactivardialogagencia();
        }).catch(err => {
          if (err.response.status == 400) {
            me.$notify("No es un usuario válido", 'error')
          } else if (err.response.status == 401) {
      ...
      listarAgenciasUsuario(){
              ...
              me.$conf.get('api/Agencias/ListarporDSP/' + me.u_iddsp,configuracion).then(function(response){
                  ...
                  agenciasArray.map(function(x){
                      me.agenciasu.push({text: x.nombre,value:x.idAgencia});
                  });
      ...
      abrircarpeta (item) {
        ...
        this.$router.push('./umixta-detallescarpetatableroa')
      },
      close () {
        this.dialog = false;
        this.limpiar()
