            ...
            <v-flex xs12 sm12 md12  >
                ...
                <v-data-table
                        ...
                        <template slot="items" class="white" slot-scope="props">
                            ...
                            <td class="caption1">{{ props.item.fechas}}</td>
                            <td class="caption1">{{ props.item.ciniciadas}}</td>
                        </template>
                        <template slot="no-data">
                        </template>
        ...
        listarporfecha(){
          ...
          me.$validator.validate().then(result => {
            if (result) {
                ...
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                        this.$cat.get('api/RHechoes/InformacionCompleta/'+me.u_iddsp+'/'+ me.fechaInicio +" 00:00:00"+'/'+me.fechaFin+" 23:59:59",configuracion).then(function(response){
                            me.carpetas=response.data;
                                ...
                                me.carpetasmodulos=response.data;
                                    ...
                                    me.carpetasmodulos.forEach(function (registro){
                                        cont += registro.ciniciadas
                                    });
                                    me.carpetasfecha.forEach(function (registro){
                                        cont2 += registro.ciniciadas
                                    });

                                    me.carpetasmodulos.push({modulo: 'Total',ciniciadas: cont });
                                    me.carpetasfecha.push({fechas: 'Total',ciniciadas: cont2 });
                                    ...
                                    me.carpetasmodulos.forEach(function (registro){
                                        if(registro.modulo!='Total'){
                                            ...
                                            total.push(registro.ciniciadas)
                                        }
                                    });
                                    ...
                                    //ciclo para extraer datos
                                    me.carpetasfecha.forEach(function (registro){
                                        if(registro.fechas!='Total'){
                                            perito.push(registro.fechas)
                                            total.push(registro.ciniciadas)
                                        }
                                    });
                                    //Creación de las dos graficas principales de cada tabla
                                    ...
                                    for(var i=0;i<me.carpetasmodulos.length-1;i++){
                                        me.crear();
                                        me.graficarindividual(me.carpetasmodulos[i].modulo,me.carpetasmodulos[i].ciniciadas);
                                        me.conteograficas++;
                                    }
                                    me.conteocoloresindiviadules = 0;
                                    for(var i=0;i<me.carpetasfecha.length-1;i++){
                                        me.crear2();
                                        me.graficarindividual(me.carpetasfecha[i].fechas,me.carpetasfecha[i].ciniciadas);
                                        me.conteograficas++;
                                    }
