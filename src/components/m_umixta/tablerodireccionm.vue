        ...
        <v-toolbar flat color="white">
          ...
          <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                        <v-btn class="mx-2 pt-2" slot="activator" v-on="on" @click="filtrarpormodulo()" fab dark small color="success">
                  ...
                  <v-card-text>
                      <v-form ref="form">
                          ...
                          <v-card-actions>
                              ...
                              <v-btn @click="filtrarModuloU()" class="success">Filtrar</v-btn>
                          </v-card-actions>
                      </v-form>
                  </v-card-text>
           </v-card>
    ...
    export default {
      ...
      methods: {
        ...
        listar () {
          ...
          this.$cat.get('api/RegistroTableroI/ListarM/' + me.u_idagencia,configuracion).then(function(response) {
        ...
        filtrarModuloU () {
          ...
          let header = { "Authorization" : "Bearer " + this.$store.state.token }
          let configuracion = { headers : header }
          this.$cat.get('api/RegistroTableroI/ListarMEspecifico/' + me.u_idagencia + '/' + me.modulou,configuracion).then(function(response) {
            me.carpetas=response.data;
            me.desactivardialogmodulo();
          }).catch(err => {
            if (err.response.status == 400) {
              me.$notify("No es un usuario válido", 'error')
            } else if (err.response.status == 401) {
        ...
        listarModulosUsuario(){
                ...
                me.$conf.get('api/ModuloServicios/ListarPorAgencia/' + me.u_idagencia,configuracion).then(function(response){
                    ...
                    modulosArray.map(function(x){
                        me.modulosu.push({text: x.nombre,value:x.idModuloServicio});
                    });
