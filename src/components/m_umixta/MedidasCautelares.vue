    ...
    <v-dialog v-model="modalAdd" fullscreen hide-overlay transition="dialog-bottom-transition">
      <v-card>
        <v-toolbar dark color="primary">
          ...
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn color="success" text @click.native="guardar()">GUARDAR INFORMACIÓN</v-btn>
          </v-toolbar-items>
              ...
              <v-layout wrap justify-space-between>                 
                <v-flex xs12 md6 lg6>
                 ...
                 <!--  <v-select
                    label="*Temporalidad:"
                    name ="temporalidad"
                    v-model="v_temporalidad"
                    :items="v_temporalidades"
                    v-validate="'required'"
  ...
  export default {
    ...
    data: () => ({
        ...
        headers: [
           ...
           /*  { text: 'Temporalidad', value: 'tiempo',  }, */
    ...
    methods:{ 
        ...
        listarImputados(){
            ...
            var imputadosArray=[];
            this.$cat.get('api/RAPs/ListarImputados/'+ me.rAtencionId,configuracion).then(function(response){
        ...
        listaMediaCautelar(){
            ...
            let header={"Authorization" : "Bearer " + this.$store.state.token};
            let configuracion= {headers : header};  
            this.$cat.get('api/MedidasCautelares/ListarPorRHecho/'+ me.rHechoId,configuracion).then(function(response){
                //console.log(response.data);
          ...
          me.$validator.validate().then(result => {
              if (result) {   
                     if (me.editedIndex > -1) {
                      ...
                      function(){
                      this.$cat.put('api/MedidasCautelares/Actualizar',{
                          'idMedCautelares': me.idMedCautelares,
                          'personaId': me.v_imputado,
                          'rHechoId': me.rHechoId,
                          'medidaCautelar': me.v_mcautelar.value != 'Otra' ?  me.v_mcautelar.value :"INEGI-" + me.otras,
            ...
            function(){
                alertify.warning('Verifica la información')
                 ...
                 debugger
                      ...
                      function(){   
                        this.$cat.post('api/MedidasCautelares/Crear',{
                            'personaId': me.v_imputado,
                            'rHechoId': me.rHechoId,
