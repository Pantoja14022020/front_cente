  ...
  <v-layout align-start>
          ...
          <v-card>
            <v-toolbar card dark color="grey lighten-4 primary--text">
              ...
              <v-toolbar-title class="subheading"
                >Reasignación de Carpetas</v-toolbar-title
              >
              <v-spacer />
            </v-toolbar>
            <v-card-text>
              <v-form ref="form">
                ...
                <v-card-actions>
                  <v-spacer />
                  <v-btn @click="desactivardialogmodulo()">Cancelar</v-btn>
                  <v-btn @click="cambioModulo" class="success">Guardar</v-btn>
                </v-card-actions>
              </v-form>
            </v-card-text>
          </v-card>
                ...
                <tr>
                  ...
                  <td style="color: #78be20">
                    <b>
                      ...
                      {{
                        fechaelevanuc.substring(8, 10) +
                        " de " +
                        obtenermes(fechaelevanuc.substring(5, 7) - 1) +
                        " del " +
                        fechaelevanuc.substring(0, 4) +
                        ", " +
                        fechaelevanuc.substring(11, 19)
                      }}
                    </b>
                  </td>
                </tr>
                  ...
                  <td style="color: #78be20">
                    <div v-if="fechasuceso != null">
                      <b>
                        <v-icon>access_time</v-icon>
                        {{
                          fechasuceso.substring(8, 10) +
                          " de " +
                          obtenermes(fechasuceso.substring(5, 7) - 1) +
                          " del " +
                          fechasuceso.substring(0, 4) +
                          ", " +
                          fechasuceso.substring(11, 19)
                        }}
                      </b>
                    </div>
                    <div v-else>
            ...
            <v-card-text>
              <v-form data-vv-scope="form1">
                <vue-editor
                  ...
                  :error-messages="
                    errors.collect('form1.narrativa de los hechos')
                  "
                  v-validate="'required'"
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      ...
                      slot="activator"
                      v-on="on"
                      @click="mostrarpdf_EntrevistaVictima()"
                      absolute
                      top
                      right
                      fab
    ...
    <v-dialog v-model="modalFHS" max-width="700px">
      <v-card>
        <v-toolbar card dark color="grey lighten-4 primary--text">
          ...
          <v-toolbar-title class="subheading"
            >Fecha y hora del suceso</v-toolbar-title
          >
          <v-spacer />
        </v-toolbar>
        <v-card-text>
          <v-form ref="form">
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs6 sm6 md6>
                  <v-date-picker locale="es" v-model="fecha" />
                </v-flex>
                <v-flex xs6 sm6 md6>
    ...
    <v-dialog v-model="modalDS" max-width="950px">
      <v-card>
        <v-toolbar card dark color="grey lighten-4 primary--text">
          <v-avatar size="30">
            <v-icon class="grey lighten-2">place</v-icon>
          </v-avatar>
          <v-toolbar-title class="subheading"
            >Dirección del suceso</v-toolbar-title
          >
          <v-spacer />
        </v-toolbar>
        <v-card-text>
            ...
            <v-layout wrap justify-space-between>
              <v-flex xs12 md5 lg5>
                ...
                <v-text-field
                  ...
                  v-model="noExt"
                />
                <v-text-field label="No.Interior:" v-model="noInt" />
                <v-text-field label="Entre calle 1:" v-model="entreCalle1" />
                <v-text-field label="Entre calle 2:" v-model="entreCalle2" />
                <v-text-field label="Referencia:" v-model="referencia" />
              </v-flex>
              <v-divider class="mx-2" inset vertical />
              <v-flex xs12 md6 lg6>
                <v-text-field
                ...
                <v-layout wrap justify-space-between>
                  <v-flex xs6 md6 lg6>
                    <v-text-field v-model="lat" label="Latitud:" disabled />
                  </v-flex>
                  <v-flex xs6 md6 lg6>
    ...
    <v-dialog
      v-model="modal_EntrevistaVictima"
      fullscreen
      hide-overlay
      transition="dialog-bottom-transition"
    >
        ...
        <v-toolbar dark color="primary">
          ...
          <v-toolbar-items>
            <v-btn
              color="success"
              text
              @click.native="imprimir_EntrevistaVictima()"
              >{{ text_Modal }}</v-btn
            >
    ...
    <v-dialog
      v-model="modal_LecturaDerechosVictima"
      fullscreen
      hide-overlay
      transition="dialog-bottom-transition"
    >
    ...
    <v-dialog v-model="dialogvanabim" max-width="1000px">
      <v-card>
        <v-toolbar card dark color="grey lighten-4 primary--text">
          ...
          <v-toolbar-title class="subheading"
            >Actualizar banavim</v-toolbar-title
          >
          <v-spacer />
        </v-toolbar>
        <v-card-text>
          <v-form ref="form">
              ...
              <v-layout row wrap>
                <v-flex xs12 sm12 md12>
                  <v-text-field
                    ...
                    v-model="vanabim"
                    v-validate="'required'"
                    @input="campoVacio = vanabim === ''"
                    :error-messages="errors.collect('banavim')"
                  />
                </v-flex>
              </v-layout>
            </v-container>
            <v-card-actions>
              <v-spacer />
              <v-btn @click="dialogvanabim = false">Cancelar</v-btn>
              <v-btn
                @click="actualizarvanabim()"
                class="success"
                v-if="!campoVacio"
                :disabled="vanabim === ''"
                >Guardar</v-btn
    ...
    <v-dialog v-model="dialostatus" max-width="1000px">
      <v-card>
        <v-toolbar card dark color="grey lighten-4 primary--text">
          ...
          <v-toolbar-title class="subheading"
            >Actualizar status y etapa de la carpeta</v-toolbar-title
          >
          <v-spacer />
        </v-toolbar>
        <v-card-text>
              ...
              <v-layout row wrap>
                ...
                <v-flex xs12 sm12 md12>
                  ...
                  <v-autocomplete
                    ...
                    v-model="status"
                    v-validate="'required'"
                    label="Etapa (OBLIGATORIO):"
                    @change="campoVacio2 = status === ''"
                    :error-messages="errors.collect('etapa')"
                  />
                </v-flex>
              </v-layout>
            </v-container>
...
export default {
  data() {
    return {
      ...
      u_claveagencia: this.$store.state.usuario.claveAQDeriva,
      personaId: 0,
      curp: "",
      nombre: "",
      alias: "",
  ...
  methods: {
    ...
    cambioModulo() {
      ...
      var descripcionRegTabI = "Registro de cambio de modulo del usuario " + me.u_nombre + " perteneciente al modulo " + me.u_modulo + " al modulo " + me.modulou.text;
    ...
    listarModulosUsuario() {
      ...
      me.$conf.get("api/ModuloServicios/ListarPorAgenciaSinCAT/" + me.u_idagencia + "/" +  me.u_claveagencia,configuracion).then(function (response) 
      {
          modulosArray = response.data;
          modulosArray.map(function (x) {
            me.modulosu.push({
              text: x.nombre,
              value: x.idModuloServicio
            });
          });
        })
        .catch((err) => 
    ...
    listarStatuscarpeta() {
      ...
      var statusArray = [];
      me.$conf
        .get("api/StatusNUCs/ListarT", configuracion)
        .then(function (response) {
    ...
    listarstatus() {
      ...
      me.$conf
        .get("api/StatusNUCs/ListarTD/" + this.etapa, configuracion)
        .then(function (response) {
          statusArray = response.data;
          statusArray.map(function (x) {
      ...
      axios
        ...
        .then(function (response) {
          ...
          if (me.narrativaHechos == null) {
            ...
            if (me.edad >= 18) {
              me.narrativaHechos =
                "<p><p><p>Esta representación social le da lectura y le explica los derechos que la Constitución Política de los Estados Unidos Mexicanos y el Código Nacional de Procedimientos Penales le concede al denunciante y/o víctima y le entrega la correspondiente acta de derechos. </p>" +
    ...
    funcionprincipal() {
      ...
      if (me.edad >= 18 && me.tvictima == "Imputado") {
        //Si mi edad es mayor a 18 o igual y soy imputado, mando a llamar esa funcion, si no la condicion que sigue y asi
        me.fila1 =
          "<p>En este acto, el (la) agente del Ministerio Público hace del conocimiento del imputado <strong>" +
          this.nombre +
          "</strong>" +
        ...
        me.edad < 18 &&
        (me.tvictima == "Victima directa" || me.tvictima == "Victima indirecta")
      ) {
        me.fila1 =
    ...
    comprobarinfo() {
      ...
      //Aqui construiere el QR
      me.qrCode = null;
      me.generarQR(me.docCaratula,me.nuc,me.u_nombre,me.fechaelevanuc,me.rHechoId);
      ...
      axios
        ...
        .then(function (response) {
          ...
          comprobarArray.map(function (x) {
            fechaCom.push({
              text: x.fechaHoraSuceso,
              value: x.fechaHoraSuceso,
            });
            direccionCom.push({ text: x.idDDelito, value: x.idDDelito });
            estatuscom.push({ text: x.etapanuc, value: x.etapanuc });
            me.clasificacioncom.push({
              text: x.clasificacionPersona,
              value: x.clasificacionPersona,
            });
            delitocom.push({ text: x.idRDH, value: x.idRDH });
          });
          ...
          delitocom.forEach(function (notificacion) {
            if (
              typeof notificacion !== null &&
              typeof notificacion !== undefined
            ) {
              if (notificacion == "[object Object]") {
                delitovacia += notificacion.text;
              } else {
                delitovacia += notificacion;
              }
            }
          });
    ...
    crearPdf_EntrevistaVictima() {
      ...
      var fecha = dia + " de " + mes + " del " + año;
      //***************************************************************************** */
      var htmltexto = htmlToPdfmake(this.narrativaHechos);
      var dd = {
        pageSize: "LEGAL",
        pageOrientation: "portrait",
        ...
        content: [
          ...
          this.edad >= 18
            ? {
                text:
                  "ENTREVISTA A CARGO DEL (A) (DENUNCIANTE, VICTIMA U OFENDIDO)\nC. " +
                  this.nombre,
                style: "titulo",
              }
            : "",
          ...
          this.edad >= 18
            ? {
                text:
                  "El (La) agente del Ministerio Público con fundamento en lo establecido por el artículo 49 del Código Nacional de Procedimientos Penales, protesta al C. " +
                  this.nombre +
                ...
                table: {
                  ...
                  body: [
                    [
                      {
                        colSpan: 3,
                        text: "DATOS DE LA VÍCTIMA U OFENDIDO",
                        fillColor: "#eeeeee",
                        fontSize: 12,
                        bold: true,
                        alignment: "center",
          ...
          this.edad < 18
            ? {
                text:
                  "El(La) agente del Ministerio Público con fundamento en lo establecido por el artículo 49 del Código Nacional de Procedimientos Penales, exhorta/informa al menor de iniciales " +
                  this.inciales +
    ...
    crearPdf_LecturaDerechosVictima() {
      ...
      var html_textofila4 = htmlToPdfmake(
        "<p>Una vez que me fueron leídos y explicados mis derechos, los comprendo y recibo una copia de los mismos, firmando al calce.</p><br><br><br><br>" +
          "<p>NOMBRE DE LA VICTIMA</p><br><p><strong>" +
          this.nombre +
          "</strong></p><br><br><br><br>" +
          "<p>A T E N T A M E N T E.</p><br>" +
          this.u_nombre +
          "<br>" +
          this.puestoquienregistro +
          "<br>" +
    ...
    listarCiudades() {
      ...
      var ciudadesArray = [];
      this.$conf
        .get("api/Estadoes/Mostrar", configuracion)
        .then(function (response) {
    ...
    listarPorEstado() {
      ...
      this.$conf
        .get("api/Municipios/ListarPorEstado/" + me.estadoid, configuracion)
        .then(function (response) {
    ...
    listarPorMunicipio() {
      ...
      this.$conf
        .get("api/Localidads/MostrarPorMPO/" + me.municipioid, configuracion)
        .then(function (response) {
    ...
    buscarPorCpMpo() {
      ...
      this.$conf
        .get(
          "api/Localidads/MostrarPorCPMpo/" + me.municipioid + "," + this.cp,
          configuracion
        )
        .then(function (response) {
          localidadArray = response.data;
    ...
    actualizarstatuscar() {
      ...
      if (me.etapa == "Suspendida" || me.etapa == "Concluida") {
        axios
          ...
          .catch((err) => {
            alert(
              "No puedes actualizar el estatus de conclusión o suspensión si no has hecho su debido registro en resolucion de carpeta"
            );
          });
      } else {
        axios
    ...
    setPlace(place) {
      (this.markers = []), (this.places = []), (this.currentPlace = place);
    },
    addMarker() {
