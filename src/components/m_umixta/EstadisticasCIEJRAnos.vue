            ...
            <v-flex xs12 sm12 md12  >
                    ...
                    <v-data-table
                            ...
                            <template  slot="items" class="white" slot-scope="props"  >
                                ...
                                <td class="caption1">{{ props.item.ciniciadas}}</td>
...
<script>
  ...
  export default {
    ...
    data: () => ({
        ...
        e403:false,
        //*************** */
        carpetasmodulos:[{modulo:"",ciniciadas:"",envio:""}],
        carpetasfecha:[{fecha:"",ciniciadas:"",envio:""}],
        carpetasmodulos2:[],
        carpetasfecha2:[],
        carpetasmodulos3:[],
        carpetasfecha3:[],
    ...
    methods:{
        ...
        graficarindividual(campo,ciniciadas,envios){
        ...
        graficarbarra(division,perito, totalc,totale){
            let me = this;
            ...
            */
            ...
            datasetf.datasets.push({
                barPercentage: 0.7,
                label:"Envíos a justicia restaurativa",
                backgroundColor: me.coloresrelleno2[0],
                borderColor: me.coloresborde2[0],
                borderWidth: 1,
                data: totale
            },)
            ...
            var myChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: 'Grafica de '+me.fechai+" a "+ me.fechaf
                    },
                    layout: {
                        padding: {
                            left: 50,
        ...
        graficarlinea(division,perito,totalc,totale){
            let me = this;
            var ctx = document.getElementById(division);
            var datasetf ={
                labels: perito,
        ...
        listarporfecha(){
          ...
          me.$validator.validate().then(result => {
            if (result) {
                let header={"Authorization" : "Bearer " + this.$store.state.token};
                let configuracion= {headers : header};
                    this.$cat.get('api/RHechoes/ContarCarpetasiniciadas/'+me.u_iddsp+'/'+ me.fechai+"-01-01 00:00:00.0000000"+'/'+me.fechaf+"-12-31 23:23:59.0000000",configuracion).then(function(response){
                    me.carpetasmodulos2=response.data;
                        this.$cat.get('api/RHechoes/ContarCarpetasiniciadasFechaAños/'+me.u_iddsp+'/'+ + me.fechai+"-01-01 00:00:00.0000000"+'/'+me.fechaf+"-12-31 23:23:59.0000000",configuracion).then(function(response){
                        me.carpetasfecha2=response.data;
                            me.$justiciarestaurativa.get('api/Envios/ContarCarpetasiniciadas/'+ me.fechai+"-01-01 00:00:00.0000000"+'/'+me.fechaf+"-12-31 23:23:59.0000000",configuracion).then(function(response){
                                me.carpetasmodulos3=response.data;
                                    me.$justiciarestaurativa.get('api/Envios/ContarCarpetasiniciadasFechaAños/'+ me.fechai+"-01-01 00:00:00.0000000"+'/'+me.fechaf+"-12-31 23:23:59.0000000",configuracion).then(function(response){
                                    me.carpetasfecha3=response.data;

                                        me.carpetasmodulos =[]
                                        me.carpetasfecha = []

                                        me.carpetasmodulos2.forEach(function (registro){
                                            me.carpetasmodulos.push({modulo:registro.modulo,ciniciadas:registro.ciniciadas,envios:0})
                                        });
                                        var entre = false;

                                        me.carpetasmodulos3.forEach(function (registro){
                                            for(var i=0;i<me.carpetasmodulos.length;i++){
                                                if(registro.modulo == me.carpetasmodulos[i].modulo){
                                                    me.carpetasmodulos[i].envios = registro.envios
                                                    entre =true;
                                                    break
                                                }
                                            }
                                            if(!entre)
                                                me.carpetasmodulos.push({modulo:registro.modulo,ciniciadas:0,envios:registro.envios})
                                                entre= false
                                        });

                                        me.carpetasfecha2.forEach(function (registro){
                                            me.carpetasfecha.push({fecha:registro.fecha,ciniciadas:registro.ciniciadas,envios:0})
                                        });
                                        var entre = false;

                                        me.carpetasfecha3.forEach(function (registro){
                                            for(var i=0;i<me.carpetasfecha.length;i++){
                                                if(registro.fecha == me.carpetasfecha[i].fecha){
                                                    me.carpetasfecha[i].envios = registro.envios
                                                    entre =true;
                                                    break
                                                }
                                            }
                                            if(!entre)
                                                me.carpetasfecha.push({fecha:registro.fecha,ciniciadas:0,envios:registro.envios})
                                                entre= false
                                        });
                                    ...
                                    me.carpetasmodulos.forEach(function (registro){
                                        ...
                                        cont3 +=registro.ciniciadas
                                    });
                                    me.carpetasfecha.forEach(function (registro){
                                        cont2 += registro.envios
                                        cont4 += registro.ciniciadas
                                    });

                                    me.carpetasmodulos.push({modulo: 'Total',ciniciadas:cont3,envios: cont });
                                    me.carpetasfecha.push({fecha: 'Total',ciniciadas:cont4,envios: cont2 });
                                    ...
                                    me.carpetasmodulos.forEach(function (registro){
                                        if(registro.modulo!='Total'){
                                            ...
                                            totalc.push(registro.ciniciadas)
                                            totale.push(registro.envios)
                                        }
                                    });
                                    //Creación de las dos graficas principales de cada tabla
                                    me.graficarbarra("myChart",perito,totalc,totale)
                                    me.graficarlinea("myChartlinea",perito,totalc,totale)
                                    ...
                                    //Creación de las dos graficas principales de cada tabla
                                    me.graficarbarra("myChart2",perito,totalc,totale)
                                    me.graficarlinea("myChartlinea2",perito,totalc,totale)

                                    //creacion de las graficas individuales
                                    for(var i=0;i<me.carpetasmodulos.length-1;i++){
                                        me.crear();
                                        me.graficarindividual(me.carpetasmodulos[i].modulo,me.carpetasmodulos[i].ciniciadas,me.carpetasmodulos[i].envios);
                                        me.conteograficas++;
                                    }
                                    me.conteocoloresindiviadules = 0;
                                    for(var i=0;i<me.carpetasfecha.length-1;i++){
                                        me.crear2();
                                        me.graficarindividual(me.carpetasfecha[i].fecha,me.carpetasfecha[i].ciniciadas,me.carpetasfecha[i].envios);
                                        me.conteograficas++;
                                    }
