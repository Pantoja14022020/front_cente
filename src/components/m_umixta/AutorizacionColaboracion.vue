      ...
      >
        <template slot="items" class="white" slot-scope="props">
          ...
          <td class="layout">
            ...
            <div v-if="props.item.status == 'Solicitado' ">
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-icon class="mr-2" v-on="on" @click="actdialogaceptar(props.item)">
                    check
                  </v-icon>
                </template>
            ...
            <div v-if="props.item.status == 'Solicitado'" >
              <v-tooltip bottom  >
                <template v-slot:activator="{ on }">
                  <v-icon class="mr-2" v-on="on" @click="actdialorechazo(props.item)">
                    close
                  </v-icon>
                </template>
                <span>Rechazar</span>
                    ...
                    <v-list two-line>
                      ...
                      <v-list-tile>
                        ...
                        <v-list-tile-content>
                          <v-list-tile-title>
                            <p class="body-2 font-weight-bold">
                              <a>Tipo de colaboración:</a>
                            </p>
                          </v-list-tile-title>
                          <v-list-tile-sub-title>
                            <p color="accent" class="caption font-weight-regular">
                              <a>{{ datos.tipoColaboracion }}</a>
                            </p>
                          </v-list-tile-sub-title>
                        </v-list-tile-content>
                      </v-list-tile>
  ...
  export default {
    ...
    methods: {
      ...
      actdialogaceptar (item) {
        this.idcolaboracion = item.idSColaboracionMP
      ...
      listar () {
        ...
        this.$cat.get('api/SColaboracionMP/ListarporAgencia/' + me.u_idagencia, configuracion).then(function(response) {
      ...
      aceptarsoli () {
        ...
        me.$validator.validateAll("Aceptar").then(result => {
          if (result) {
            this.$cat.post('api/AsignacionColaboracion/Crear', {
              'ModuloServicioId': me.modulo.value,
              'SColaboracionMPId':me.idcolaboracion,
      ...
      rechazarsoli () {
        ...
        me.$validator.validateAll("Rechazar").then(result => {
          if (result) {
            this.$cat.put('api/SColaboracionMP/Actualizarrechazo', {
              'IdSColaboracionMP': me.idcolaboracion,
              'Respuesta': me.rechazo,
              'Status': "Rechazado",
            },configuracion).then(function(response) {
              me.$notify('La información se actualizo correctamente !!!', 'success')
              me.limpiar()
